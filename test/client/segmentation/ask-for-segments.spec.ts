...
describe( 'Directive askForSegments when template is select-segments.html', ()=>{
	var $compile: angular.ICompileService;
	var scope: CustomScope;
	var controller: angular.IControllerService;
	var directiveScope: Segmentation.IAskForSegmentScope;
	...
	var html = '<div ask-for-segments="config" template="client/segmentation/select-segments.html"></div>';
	var element: angular.IAugmentedJQuery;
	...
	// load the templates
	beforeEach( angular.mock.module( 'client/segmentation/select-segments.html') );
	...
	it('should show only segment type selector',()=>{
		expect( element.find('select').first().children().length ).toBeLessThan(2);
		expect( element.find('select').last().children().length ).toBeLessThan(2);
	});
	...
	describe('when grouped selected',()=>{
		...
		it('sould show fields list', ()=>{
			expect( element.find('select').first().children().length ).toBe( 4 );
			expect( element.find('select').last().children().length).toBeLessThan(2);
		});

		it('should show description',()=>{
			expect( element.find('select>option').first().text().trim() ).toBe('Pais');
		});
	...
	describe('when opened_emails selected',()=>{
		...
		it('sould show followups and broadcast list', ()=>{
			expect( element.find('select').first().children().length ).toBeLessThan( 2 );
			expect( element.find('select').last().children().length).toBe(4);
		});

		it('should show description',()=>{
			expect( element.find('select').last().children('option').first().text().trim() ).toBe('#25 - Esto es un email');
		});
